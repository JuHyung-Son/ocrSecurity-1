...
def get_cocotext_recognizer_dataset(split='train',
    ...
    labels_zip = utils.download_and_verify(
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for future in tqdm.tqdm(concurrent.futures.as_completed([
                executor.submit(utils.download_and_verify,
...
def get_born_digital_recognizer_dataset(split='train', cache_dir=None):
    ...
    if split in ['train', 'traintest']:
        ...
        training_zip_path = utils.download_and_verify(
            url=
    ...
    if split in ['test', 'traintest']:
        ...
        test_zip_path = utils.download_and_verify(
            url=
        ...
        test_gt_path = utils.download_and_verify(
...
def get_icdar_2013_recognizer_dataset(cache_dir=None):
    ...
    for image_path, lines, _ in get_icdar_2013_detector_dataset(cache_dir=cache_dir,
                                                                skip_illegible=True):
        for line in lines:
            box, text = utils.combine_line(line)
...
def get_icdar_2013_detector_dataset(cache_dir=None, skip_illegible=False):
    ...
    training_zip_images_path = utils.download_and_verify(
        url=
    ...
    training_zip_gt_path = utils.download_and_verify(
...
def get_detector_image_generator(labels, width, height, augmenter=None, area_threshold=0.5):
    ...
    for index in itertools.cycle(range(len(labels))):
        ...
        image = utils.read(image_filepath)
        if augmenter is not None:
            image, lines = utils.augment(boxes=lines,
                                         boxes_format='lines',
                                         image=image,
                                         area_threshold=area_threshold,
                                         augmenter=augmenter)
        image, scale = utils.fit(image,
                                 width=width,
                                 height=height,
                                 mode='letterbox',
                                 return_scale=True)
...
def get_recognizer_image_generator(labels, height, width, alphabet, augmenter=None):
    ...
    for index in itertools.cycle(range(len(labels))):
        ...
        if box is not None:
            image = utils.warpBox(image=utils.read(filepath),
                                  box=box.astype('float32'),
                                  target_height=height,
                                  target_width=width,
                                  cval=cval)
        else:
            image = utils.read_and_fit(filepath_or_array=filepath,
                                       width=width,
                                       height=height,
                                       cval=cval)
