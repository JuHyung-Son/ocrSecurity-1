...
def drawAnnotations(image, predictions, ax=None):
    ...
    if ax is None:
        _, ax = plt.subplots()
    ax.imshow(drawBoxes(image=image, boxes=predictions, thickness=1, boxes_format='predictions'))
    predictions = sorted(predictions, key=lambda p: p[1][:, 1].min())
    table = []
    for word, box in predictions:
        table.append((word,box))
    ax.set_yticks([])
    ax.set_xticks([])


    for index, (text, box) in enumerate(table):
        y = 1 - (index / len(table))
        ax.text(box[0][0], box[0][1], "{}".format(index), color='blue', fontsize=20,
...
def drawBoxes(image, boxes, color=(255, 0, 0), thickness=5, boxes_format='boxes'):
    ...
    Args:
        ...
        boxes_format: The format used for providing the boxes. Options are
            ...
            by `ocr7.detection.Detector.detect`, "lines" (a list of
            lines where each line itself is a list of (box, character) tuples) as
            provided by `ocr7.data_generation.get_image_generator`,
            or "predictions" where boxes is by itself a list of (word, box) tuples
            as provided by `ocr7.pipeline.Pipeline.recognize` or
            `ocr7.recognition.Recognizer.recognize_from_boxes`.
    """
    if len(boxes) == 0:
        return image
    canvas = image.copy()
