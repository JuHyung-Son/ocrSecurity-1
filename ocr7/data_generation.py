...
def get_backgrounds(cache_dir=None):
    ...
    backgrounds_zip_path = utils.download_and_verify(
...
def get_fonts(cache_dir=None,
    ...
    if cache_dir is None:
        cache_dir = os.path.expanduser(os.path.join('~', '.keras-ocr'))
    fonts_zip_path = utils.download_and_verify(
...
def convert_image_generator_to_recognizer_input(image_generator,
    ...
    while True:
        ...
        for line in lines:
            ...
            if not line:
                continue
            box, sentence = utils.combine_line(line)
            crop = utils.warpBox(image=image,
                                 box=box,
...
def get_image_generator(height,
    ...
    for text, background_index, current_font_groups in zip(
        ...
        current_background = utils.read(current_background_filepath_or_array) if isinstance(
        ...
        if current_background.shape[0] != height or current_background.shape[1] != width:
            current_background = utils.fit(current_background,
                                           width=width,
                                           height=height,
