...
from models.model_train_detector import Model_Train
...
parser.add_argument("--checkpoint_dir", default="/root/optimization/ocrSecurity/ocr_onnx/outputs/detector/checkpoints/", type=str,help = "Dir for checkpoints")
parser.add_argument("--summary_dir", default="/root/optimization/ocrSecurity/ocr_onnx/outputs/detector/summaries/", type=str,help = "Dir for tensorboard logs.")
...
config.checkpoint_dir += "OCR_" + config.model_tag; check_folder(config.checkpoint_dir)
...
for e in range(config.epoch):
    for i, image_features in enumerate(dataset):
        ...
        #
        log = model.train_step(image_features)
