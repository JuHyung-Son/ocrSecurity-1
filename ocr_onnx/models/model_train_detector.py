...
class Model_Train():
    ...
    def save(self,epoch):
        ...
        self.model.summary()
        self.model_save_path = os.path.join(self.config.checkpoint_dir, "generator_scale_{}.h5".format(epoch))
        self.model.save(self.model_save_path)
        restored_model = tf.keras.models.load_model(self.model_save_path, compile=False)
        onnx_model = keras2onnx.convert_keras(restored_model, restored_model.name)
        keras2onnx.save_model(onnx_model, '/root/optimization/ocrSecurity/model.onnx')
